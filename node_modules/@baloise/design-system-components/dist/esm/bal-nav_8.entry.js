...
import { L as ListenToMutation } from './mutation.decorator-383504dd.js';
import './breakpoints.subject-89817b34.js';
import { L as ListenToBreakpoints } from './breakpoints.decorator-372e9eb8.js';
import { b as balBrowser } from './browser-9199b5e2.js';
import { B as BalScrollHandler } from './scroll-5f7701b9.js';
import { d as defaultConfig } from './initialize-ac68e3e3.js';
import { L as ListenToConfig } from './config.decorator-54721e29.js';
import { w as waitForComponent, e as waitAfterIdleCallback } from './helpers-697ce790.js';
import { L as ListenToResize } from './resize.decorator-5bed5c0e.js';
...
class NavLinkItem {
  ...
  renderTouch(_context) {
    ...
    return (h("li", null,
      h("a", { id: this.id, class: Object.assign(Object.assign(Object.assign({}, block.element('mobile-links').class()), block.element('mobile-links').modifier('selected').class(this.active)), block.element('mobile-links').modifier('clickable').class(this.clickable)), href: this.href, target: this.target, onClick: ev => this.onClick(ev) }, this.label)));
  }
  render(_context) {
...
class NavSectionLinkItem extends NavLinkItem {
  ...
  renderTouch(_context) {
    ...
    return (h("li", null,
      h("a", { id: this.id, class: Object.assign(Object.assign(Object.assign({}, block.element('mobile-section-item').class()), block.element('mobile-section-item').modifier('selected').class(this.active)), block.element('mobile-section-item').modifier('clickable').class(this.clickable)), href: this.href, target: this.target, onClick: ev => this.onClick(ev) }, this.label),
      hasItems ? (h("ul", { class: Object.assign({}, block.element('mobile-links-list').class()) }, this.linkItems.map(item => item.renderTouch()))) : ('')));
  }
  render(_context) {
...
class NavServiceLinkItem extends NavLinkItem {
  ...
  renderTouch(_context) {
    ...
    const hasItems = this.linkItems && this.linkItems.length > 0;
    return (h("li", null,
      h("a", { id: this.id, class: Object.assign(Object.assign(Object.assign({}, block.element('mobile-service-item').class()), block.element('mobile-service-item').modifier('selected').class(this.active)), block.element('mobile-service-item').modifier('clickable').class(this.clickable)), href: this.href, target: this.target, onClick: ev => this.onClick(ev) }, this.label),
      hasItems ? (h("ul", { class: Object.assign({}, block.element('mobile-links-list').class()) }, this.linkItems.map(item => item.renderTouch()))) : ('')));
  }
  render(_context) {
...
const AccordionButton = ({ id, label, level, open, link, href, target, onClick, }) => {
  const block = BEM.block('nav');
  return link ? (h("a", { id: id, class: Object.assign({}, block.element(`mobile-${level}-item`).class()), href: href, target: target, onClick: ev => onClick(ev) },
...
const OverviewLink = ({ item, isMenu, onClick }) => {
  const block = BEM.block('nav');
  if (!item) {
    return '';
  }
  return (h("a", { class: Object.assign(Object.assign({}, block.element('mobile-overview-link').class()), block.element('mobile-overview-link').modifier('menu').class(!!isMenu)), href: item.href, target: item.target, onClick: ev => onClick(ev) }, item.label));
};
...
class NavMenuLinkItem extends NavLinkItem {
  ...
  renderTouch(context) {
    ...
    return (h("li", null,
      h(AccordionButton, { id: this.id, level: "menu", label: this.label, href: this.href, target: this.target, link: this.isLink, open: isSelected, onClick: ev => this.onAccordionClick(ev) }),
      isSelected ? (h(OverviewLink, { item: this.overviewLink, onClick: ev => this.onClick(ev), isMenu: true })) : (''),
      isSelected && hasSectionLinkItems ? (h("ul", { class: Object.assign({}, block.element('mobile-section-list').class()) }, this.sectionLinkItems.map(item => item.renderTouch()))) : (''),
      isSelected && hasServiceLinkItems ? (h("ul", { class: Object.assign({}, block.element('mobile-service-list').class()) }, this.serviceLinkItems.map(item => item.renderTouch()))) : ('')));
  }
  render(context) {
    if (this.isLink) {
...
class NavMetaLinkItem extends NavLinkItem {
  ...
  renderTouch(context) {
    ...
    return (h("li", null,
      h(AccordionButton, { id: this.id, level: "meta", label: this.label, open: isSelected, link: this.isLink, href: this.href, target: this.target, onClick: ev => this.onAccordionClick(ev) }),
      isSelected ? h(OverviewLink, { item: this.overviewLink, onClick: ev => this.onClick(ev) }) : '',
      isSelected ? (h("hr", { class: Object.assign(Object.assign({}, block.element('line').class()), block.element('line').modifier('up').class()) })) : (''),
      isSelected && this.mainLinkItems && this.mainLinkItems.length > 0 ? (h("ul", { class: Object.assign({}, block.element('mobile-menu-list').class()) }, this.mainLinkItems.map(item => item.renderTouch({
...
const NavMetaBar$1 = class {
  constructor(hostRef) {
    ...
    this.hasRenderedWithFlyoutActive = false;
  ...
  async clickOnOutside(ev) {
    ...
    if (this.isDesktop) {
      if (this.hasRenderedWithFlyoutActive) {
        const targetIsInMetaBar = (_b = (_a = this.metaBarEl) === null || _a === void 0 ? void 0 : _a.querySelector('.bal-nav-meta-bar__container')) === null || _b === void 0 ? void 0 : _b.contains(ev.target);
        const targetIsInMenuBar = (_d = (_c = this.menuBarEl) === null || _c === void 0 ? void 0 : _c.querySelector('.bal-nav-menu-bar__container')) === null || _d === void 0 ? void 0 : _d.contains(ev.target);
