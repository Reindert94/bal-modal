...
import { d as dateSeparator } from './index.esm-586fc0cd.js';
...
class BalDate {
  static fromAnyFormat(value) {
    ...
    if (length === 2) {
      ...
      return new BalDate(luxon.DateTime.fromFormat(`${pairs[0]}${separator}${pairs[1]}${separator}${year}`, `d${separator}M${separator}yy`, {
        locale,
      }));
    }
    if (length === 3) {
      if (pairs[2].length === 1) {
        return new BalDate(luxon.DateTime.fromFormat(`${pairs[0]}${separator}${pairs[1]}${separator}${2000 + parseInt(pairs[2], 10)}`, `d${separator}M${separator}yy`, {
          locale,
        }));
      }
      else if (pairs[2] === '') {
    ...
    return new BalDate(luxon.DateTime.fromFormat(value, `d${separator}M${separator}yy`, { locale }));
  }
  static fromISO(value) {
    return new BalDate(luxon.DateTime.fromISO(value || ''));
  }
  static infoMonths({ format, locale } = {}) {
    const config = useBalConfig();
    return luxon.Info.months(format || 'long', { locale: locale || (config === null || config === void 0 ? void 0 : config.locale) || 'de-CH' });
  }
  static infoWeekdays({ format, locale } = {}) {
    const config = useBalConfig();
    const weekdays = luxon.Info.weekdays(format, { locale: locale || (config === null || config === void 0 ? void 0 : config.locale) || 'de-CH' });
    return weekdays;
  }
  constructor(dt) {
    this.dt = dt;
  ...
  isBefore(isoDate) {
    const date = luxon.DateTime.fromISO(isoDate || '');
    if (date.isValid) {
      return this.dt < date;
    }
    return false;
...
export { BalDate as B, i18nBalDate as i };
