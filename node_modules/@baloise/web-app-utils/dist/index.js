...
const padStart__default = /*#__PURE__*/_interopDefaultLegacy(padStart);
const isString__default = /*#__PURE__*/_interopDefaultLegacy(isString);
const isBoolean__default = /*#__PURE__*/_interopDefaultLegacy(isBoolean);
const isObject__default = /*#__PURE__*/_interopDefaultLegacy(isObject);
const isDate__default = /*#__PURE__*/_interopDefaultLegacy(isDate);
const isNaN__default = /*#__PURE__*/_interopDefaultLegacy(isNaN);
...
function areArraysEqual(a, b) {
    if (isNil__default["default"](a) || isNil__default["default"](b)) {
        return false;
    }
    const copyA = [...a].sort();
    const copyB = [...b].sort();
    return isEqual__default["default"](copyA, copyB);
}
...
function formatDateString(date) {
    return date && dateFns.isValid(date) ? dateFns.formatISO(date, { representation: 'date' }) : '';
}
/**
...
function isValidIsoString(dateString) {
    return !!dateString ? dateFns.isMatch(dateString, ISO_PATTERN) : false;
}
/**
...
function format(locale = 'de-CH', date) {
    return dateFns.isValid(date) ? intlFormat(dateLocale(locale), date) : '';
}
/**
...
function parse(dateString, locale = 'de-CH') {
    if (dateFns.isMatch(dateString, ISO_PATTERN)) {
        const d = dateFns.parseISO(dateString + TIMEZONE);
        if (d && dateFns.isValid(d)) {
            return validateYear(d);
        }
        const [year, month, day] = `${dateString}`.split('-').map(d => parseInt(d, 10));
        return generateIsoDate([year, month, day]);
    }
    if (dateFns.isMatch(dateString, getDatePattern(locale))) {
        const d = dateFns.parse(dateString, getDatePattern(locale), now());
        return generateIsoDate([dateFns.getYear(d), dateFns.getMonth(d) + 1, dateFns.getDate(d)]);
    }
    return undefined;
}
/**
...
function pad(value) {
    return padStart__default["default"](`${value}`, 2, '0');
}
function validateYear(date) {
    if (date && dateFns.isValid(date)) {
        if (dateFns.getYear(date) < 1000) {
            return dateFns.setYear(date, dateFns.getYear(date) + 2000);
        }
        return date;
    }
    return undefined;
...
function generateIsoDate([year, month, day]) {
    if (year > 0 && month > 0 && day > 0) {
        return dateFns.parseISO(`${year < 1000 ? year + 2000 : year}-${pad(month)}-${pad(day)}` + TIMEZONE);
    }
    return undefined;
}
...
function isEmpty(value) {
    ...
    if (isString__default["default"](value) && value === '') {
        return true;
    }
    if (isBoolean__default["default"](value)) {
        return false;
    }
    if (isDate__default["default"](value)) {
        return false;
    }
    if (Array.isArray(value) && value.length === 0) {
        return true;
    }
    if (isObject__default["default"](value) && Object.keys(value).length === 0) {
        return true;
    }
    return false;
}
...
function isValidMonetaryNumber(stringValue) {
    if (isNil__default["default"](stringValue) && !isString__default["default"](stringValue)) {
        return false;
    }
    ...
    stringValue = stringValue.replace(/’/g, '');
    stringValue = stringValue.replace(/,/g, '.');
    return !isNaN__default["default"](parseFloat(stringValue));
}
/**
...
exports.ISO_PATTERN = ISO_PATTERN;
exports.ITALIAN = ITALIAN;
exports.LUXEMBOURGISH = LUXEMBOURGISH;
exports.Language = Language;
exports.LanguageModel = LanguageModel;
exports.LanguagesOfBelgium = LanguagesOfBelgium;
exports.LanguagesOfGermany = LanguagesOfGermany;
exports.LanguagesOfLuxembourg = LanguagesOfLuxembourg;
exports.LanguagesOfSwitzerland = LanguagesOfSwitzerland;
exports.TIMEZONE = TIMEZONE;
exports.areArraysEqual = areArraysEqual;
exports.ceilTime = ceilTime;
exports.dateLocale = dateLocale;
exports.dateSeparator = dateSeparator;
exports.debounce = debounce;
exports.disableDragAndDropFiles = disableDragAndDropFiles;
...
exports.isValidMonetaryNumber = isValidMonetaryNumber;
exports.loadFooterLinks = loadFooterLinks;
exports.loadOneTrustBaloiseSwitzerland = loadOneTrustBaloiseSwitzerland;
...
exports.scrollToAnchorElement = scrollToAnchorElement;
exports.scrollToTopOfBody = scrollToTopOfBody;
exports.today = today;
